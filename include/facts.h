...
#define FACT_CHECK_CERR(a, op, b) (((a)op(b)) ? (++facts_truths, 1) : (FactsFiction(__FILE__, __LINE__, facts), std::cerr << __FILE__ << " " << __LINE__ << ": " << #a << " {=" << (a) << "} " << #op << " " << #b << " {=" << (b) << "} is " FACTS_RED "fiction" FACTS_RESET << std::endl, facts->status = -1, 0))
