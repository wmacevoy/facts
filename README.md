...
In a `FACTS(AboutThing) { ... }` fact-check function, you can (only in C++) add `FACTS_TRACE(your << ad << here)`.  These are printed in reverse order if a fact-check fails.  This is helpful to include additional information (like the case if looping over cases).  For example:
```C++
FACTS(...) {
  for (int i=0; i<3; ++i) {
    FACTS_TRACE("i=" << i);
